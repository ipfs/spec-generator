...
describe('General MD/HTML Processing', function () {
  ...
  it('sets the title', async () => {
    // When title is in frontmatter, it's plain text without markdown formatting
    const doc = await md2doc('# The `fun` spec', { title: 'The fun spec' });
    const selectron = makeSelectron(doc);
    selectron('body > header > h1');
    selectron('h1', 'The fun spec');
    // Note: <code> tag not present when title comes from frontmatter
  ...
  it('sets the date', async () => {
    ...
    const doc = await md2doc('# An old doc', { title: 'An old doc', lastModified: new Date(bd) });
    const selectron = makeSelectron(doc);
  ...
  it('sets maturity', async () => {
    const doc = await md2doc('# A stable doc', { title: 'A stable doc', maturity: 'stable' });
    const selectron = makeSelectron(doc);
  ...
  it('produces sections', async () => {
    const doc = await md2doc(sectionDoc, { title: 'One' });
    const selectron = makeSelectron(doc);
    selectron('section', 7);
...
async function md2doc (md, opt = {}) {
  ...
  // Don't provide editors for tests - acknowledgments section won't be created
