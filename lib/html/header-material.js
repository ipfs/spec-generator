...
export default async function run (doc, opt, ctx) {
  const el = makeEl(doc);
  let existingH1 = doc.querySelector('h1');
  let h1;
  if (opt.title) {
    h1 = el('h1', {}, [opt.title]);
    // Only remove existing H1 if its text content matches the frontmatter title
    // This preserves H1s that are intentionally different from the document title
    if (existingH1 && existingH1.textContent === opt.title) {
      existingH1.remove();
    }
  } else {
    h1 = el('h1', {}, ['Untitled']);
