...
export function addId (elem, pfx = '', txt = '', noLC = false) {
  ...
  else if (/\.$/.test(id) || !/^[a-z0-9]/i.test(pfx || id)) {
...
// When you use el.querySeletor(':scope > foo') JSDOM actually uses the ID of that element
// to turn that into el.querySeletor('el#bar > foo'). If you have a valid ID per HTML that
// isn't acceptable as a CSS selector, that blows up. All of our uses of :scope were for
// picking out in the children, so we're using this instead.
export function scopeOne (parent, selector) {
  return [...parent.children].find(el => el.matches(selector));
}

export function scopeAll (parent, selector) {
  return [...parent.children].filter(el => el.matches(selector));
}
