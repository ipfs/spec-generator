...
export default async function run (doc, opt, ctx) {
  ...
  // Check if there's anything to acknowledge
  const hasEditors = opt.editors && opt.editors.length > 0;
  const hasFormerEditors = opt.former_editors && opt.former_editors.length > 0;
  const hasContributors = opt.contributors && opt.contributors.length > 0;
  const hasThanks = opt.thanks && opt.thanks.length > 0;

  // Only create acknowledgments section if there's something to show
  if (!hasEditors && !hasFormerEditors && !hasContributors && !hasThanks) {
    // Still warn about missing editors as it's required for specs
  ...
  const all = [...(opt.editors || []), ...(opt.former_editors || []), ...(opt.contributors || []), ...(opt.thanks || [])];
  if (all.length > 1) {
  ...
  if (hasContributors) {
    appendIndividuals(el, metaEls, opt.contributors, `Contributor${opt.contributors.length > 1 ? 's' : ''}`, ctx)
  }

  if (hasThanks) {
    appendIndividuals(el, metaEls, opt.thanks, 'Special Thanks', ctx)
  }
...
function appendIndividuals (el, metaEls, optList, groupTitle, ctx) {
  ...
  optList.forEach(
    ({ name, email, url, github, twitter, mastodon, bluesky, affiliation: { name: affiliationName, url: affiliationURL } = {}}) => {
      if (!name) {
      ...
      if (bluesky) {
        person.push(' ');
        person.push(el('a', { href: `https://bsky.app/profile/${bluesky.replace(/^@/, '')}` }, [
          el('img', { src: '/img/bluesky.png', width: '16', height: '16', alt: 'Bluesky' })
        ]));
      }
