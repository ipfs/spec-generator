...
export default async function run (doc, opt, ctx) {
  ...
  if (opt && opt.xref) {
    for (const defSrc of opt.xref.reverse()) {
      try {
        ...
        ctx.error(`Could not load data for spec "${defSrc}"`);
      }
    }
  }
  ...
  doc.querySelector('body > section:last-child').after(sec);
  const citeDefs = [];
...
async function indexDefinitions (shortName) {
  const data = await loadJSON(rel(`../../refs/webref/ed/dfns/${shortName}.json`));
  const defs = {};
  data
    .dfns
