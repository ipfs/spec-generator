...
const definitionSources = [
  'https://w3c.github.io/webref/ed/dfns/',
  'https://specs.ipfs.tech/.well-known/definitions/',
];
const referenceSources = [
  'https://specs.ipfs.tech/.well-known/references/',
];
  ...
  for (const key of citeKeys) {
    try {
      if (citeCache[key]) allCites[key] = citeCache[key];
        ...
        // XXX
        // This doesn't work, refs x sources is too many requests and is slow
        // store a big one per site, get that, and find in there.
        allCites[key] = await Promise.any(referenceSources.map(src => `${src}${key}.json`).map(must200))
        citeCache[key] = allCites[key];
      }
    }
    catch (err) {
...
export async function indexDefinitions (xref, definitions) {
  const defData = await Promise.all(xref.reverse().map(defsForShortname));
  defData
...
export async function defsForShortname (shortName) {
