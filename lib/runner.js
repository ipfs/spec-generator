...
export default class IpseityRunner {
  constructor ({ input, output, template, baseURL, runMode = 'build', quietMode = false } = {}) {
    console.log(`QUIET`, quietMode);
    this.elev = new Eleventy(input, output, {
      ...
      config: (config) => {
        ...
        const processor = new IpseityProcessor({ template, output, baseURL, quietMode });
