...
export default class IpseityProcessor {
  ...
  async generateHTML (body, opt, ctx) {
    ...
    const citesPath = join(this.output, `.well-known/ipseity/references/`);
    const defsPath = join(this.output, `.well-known/ipseity/definitions/`);
    await mkdir(citesPath, { recursive: true });
    ...
    await saveJSON(join(citesPath, `${shortName}.json`), citeMeta);
    await saveJSON(join(defsPath, `${shortName}.json`), defsMeta);
