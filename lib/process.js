...
export default class IpseityProcessor {
  constructor ({ template, output, baseURL }) {
  ...
  async generateHTML (body, opt, ctx) {
    ...
    const citesFile = join(this.output, 'references.json');
    const defsFile = join(this.output, 'definitions.json');
    const citesDB = await this.maybeLoad(citesFile);
    const defsDB = await this.maybeLoad(defsFile);
    ...
    citesDB[shortName] = {
      ...
      shortName,
      href: `${this.baseURL}${opt.page.url}`,
    };
    defsDB[shortName] = ctx.definitions;
    await saveJSON(citesFile, citesDB);
    await saveJSON(defsFile, defsDB);
